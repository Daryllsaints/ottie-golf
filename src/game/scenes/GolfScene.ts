...
export class GolfScene extends Scene {
    ...
    update(_t: number, dt: number) {
        ...
        if (this.state === 'IN_FLIGHT' && !this.holeSunk) {
            // Trail stores ACTUAL ground positions. Top-down view has
            // no Z axis to sell a vertical lift, so the arc reads as
            // the curve of the ball's path across the course.
            this.trail.push({ x: bx, y: by, t: this.time.now });

            // Motion stretch along velocity direction so fast shots
            // feel fast.
            const v = (this.ballBody as unknown as { velocity: { x: number; y: number } }).velocity;
            const speed = Math.hypot(v.x, v.y);
            const stretch = Math.min(1.4, 1 + speed * 0.04);
    ...
    private drawTrail() {
        ...
        // Subtle dashed shot-line through the ball's path. Each pair
        // of consecutive samples draws a short segment whose alpha
        // fades with age; together they read as a clean curve trace
        // rather than a thick painted stripe.
        for (let i = 1; i < this.trail.length; i++) {
            const a = this.trail[i - 1];
            const b = this.trail[i];
            const age = (now - b.t) / LIFETIME;
            const alpha = (1 - age) * 0.55;
            this.trailGfx.lineStyle(1.5, COLORS.ballTrail, alpha);
